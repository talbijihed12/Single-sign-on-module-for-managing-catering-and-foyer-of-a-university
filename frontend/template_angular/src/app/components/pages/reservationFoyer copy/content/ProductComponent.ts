...
export class ProductComponent implements OnInit {
  ...
  c!:number;
  constructor(private loginService:LoginService,private route: ActivatedRoute,private singleReservationConsumerService:SingleReservationFoyerCosumerService,private _productService: ProductService, private calcul:CalculService,private productConsumer:ProductCosumerService) { }
  ...
  ngOnInit(): void {
    ...
    this.productConsumer.getProductsRF().subscribe(
      singleReservationFoyer => {
        this.products = singleReservationFoyer;
        this.products = this.products.filter(room => (room.username == this.loginService.getUsername()));
        console.log(this.products);
